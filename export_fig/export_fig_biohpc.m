function [imageData, alpha] = export_fig_biohpc(varargin)
%EXPORT_FIG_BIOHPC  Exports figures updated for usage of BioHPC at UTSW (no graphic card so rendering with openGL does not work)
%
% Modified from export_fig by Yi Du and Assaf Zaritsky, October, 2015.
%
% Examples:
        ...
        if isbitmap(options)
            ...
            if options.transparent && (options.png || options.alpha)
                ...
                try
                    ...
                    B = print2array_biohpc(fig, magnify, renderer);
                    % Downscale the image
                    B = downsize(single(B), options.aa_factor);
                catch
                    % This is more conservative in memory, but kills transparency (issue #58)
                    B = single(print2array_biohpc(fig, magnify/options.aa_factor, renderer));
                end
                ...
                try
                    % Print large version to array
                    A = print2array_biohpc(fig, magnify, renderer);
                    % Downscale the image
                    A = downsize(single(A), options.aa_factor);
                catch
                    % This is more conservative in memory, but kills transparency (issue #58)
                    A = single(print2array_biohpc(fig, magnify/options.aa_factor, renderer));
                end
                ...
                if options.transparent
                    ...
                    [A, tcol] = print2array_biohpc(fig, magnify, renderer);
                end
                % Crop the background
                if options.crop
