...
% Checked in newest version: Hunter Elliott, 5/9/2010
%
%
%
% Theory of operation:
...
if robust
	...
	if nargout > 2
		...
		sel = sel_index(index);
        if ~any(isnan(T(1,sel))) %HLE - added this because occasionally T has NaNs, causing the returned index to be NaN
            iout = i(sel) + T(1,sel).';
        else
            iout = i(sel)+.01; %HLE - Shift slightly to differentiate between true integer-valued intersections
        end
        if ~any(isnan(T(2,sel)))
            jout = j(sel) + T(2,sel).';
        else
            jout = j(sel)+.01;
        end
