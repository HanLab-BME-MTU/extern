    ...
    B. Set the path by doing one of the following:
        ...
        * Add the MCR directory to the environment variable by issuing 
        ...
        NOTE: When deploying standalone applications, it is possible 
              to run the shell script file run_DVImgOpen.sh 
...
3. Collect the following files, based on what you are deploying: 
    ...
    NOTE: DVImgOpen.ctf will be present only if you have chosen 
    ...
    All Platforms:
        * DVImgOpen.ctf (if present)(Component Technology File archive,
          platform-dependent on target machine)

    Windows:
        * MCRInstaller.exe (self-extracting MATLAB Compiler Runtime
          installer)
        * DVImgOpen.exe (application created by MATLAB Compiler)

    UNIX:
        * MCRInstaller.bin (MATLAB Compiler Runtime installer)
        * DVImgOpen (application created by MATLAB Compiler)
        * run_DVImgOpen.sh (shell script that can be run to 
    ...
    All Platforms:
        ...
        * DVImgOpen.dll (shared library, file extension varies by 
          platform)
        * DVImgOpen.h (library header file)
    ...
    Windows:
        * DVImgOpen.ctf (if present)(Component Technology File archive, contains 
          compiled M-code)    
        * DVImgOpen.xml (documentation files)
        * DVImgOpen.pdb (program database file - if DEBUG option 
          selected)
        * DVImgOpen.dll (component assembly file)
        * MCRInstaller.exe (self-extracting MATLAB Compiler Runtime
          archive)
    ...
    Windows:
        * DVImgOpen.ctf (if present)(Component Technology File archive, contains 
          compiled M-code)
        * DVImgOpen_1_0.dll (COM component)
        * MCRInstaller.exe (self-extracting MATLAB Compiler Runtime
          archive)      
    ...
    All Platforms: 
        * DVImgOpen.jar (Java class files and compiled M-code)
    ...
    Windows:
        * DVImgOpen.ctf (if present)(Component Technology File archive, contains 
          compiled M-code)
        * DVImgOpen_1_0.dll (COM component)
        * DVImgOpen.xla (optionally created and placed in
