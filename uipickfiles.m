...
%               '.m' and '.mat'.  Default is empty string.  Can be combined
%               with other filters.  Can be changed with the GUI.
%               
%   EXFilter    String containing a regular expression used to inversely
%               filter the file list. Ex: '\.m$|\.mat$' will exclude files 
%               ending in '.m' and '.mat'.  Default is empty string. Can be
%               combined with other filters.  Can be changed with the GUI. 
%
...
%
% 28.8.08, Jonas Dorn: Added exclusion filter, fixed bugs
...
if isdir(prop.filterspec)
    prop.filterspec(end+1) = filesep;
end
...
ex_filter = prop.exfilter;
full_filter = fullfile(current_dir,filter);
path_cell = path2cell(current_dir);
fdir = filtered_dir(full_filter,re_filter,ex_filter);
filenames = {fdir.name}';
...
uicontrol('Position',[10 425 80 16],...
	...
	'String','File Filter',...
	'HorizontalAlignment','left')
uicontrol('Position',[100 425 200 16],...
	'Style','text',...
	'String','Include regexp',...
	'HorizontalAlignment','left')
uicontrol('Position',[310 425 200 16],...
	'Style','text',...
	'String','Exclude regexp',...
	'HorizontalAlignment','left')
showallfiles = uicontrol('Position',[520 405 100 20],...
	'Style','checkbox',...
...
filter_ed = uicontrol('Position',[10 405 80 20],...
	...
	'HorizontalAlignment','left',...
	'Callback',@setfilspec);
refilter_ed = uicontrol('Position',[100 405 200 20],...
	'Style','edit',...
	'BackgroundColor','w',...
	'String',re_filter,...
	'HorizontalAlignment','left',...
	'Callback',@setrefilter);
exfilter_ed = uicontrol('Position',[310 405 200 20],...
	'Style','edit',...
	'BackgroundColor','w',...
	'String',ex_filter,...
	'HorizontalAlignment','left',...
	'Callback',@setexfilter);
...
uicontrol('Position',[380 375 350 20],...
	'Style','text',...
...
switch prop.output
	...
	case 'cancel'
        warning('this case should no longer be occurring. It could lead to typecasting problems')
		out = 0;
end
...
% -------------------- Callback functions --------------------
	...
	function togglefilter(varargin)
		...
		if value
			...
			re_filter = '';
            ex_filter = '';
			set([filter_ed,refilter_ed,exfilter_ed],'Enable','off')
		else
			filter = get(filter_ed,'String');
			re_filter = get(refilter_ed,'String');
            ex_filter = get(exfilter_ed,'String');
			set([filter_ed,refilter_ed,exfilter_ed],'Enable','on')
		end
		full_filter = fullfile(current_dir,filter);
		fdir = filtered_dir(full_filter,re_filter,ex_filter);
		filenames = {fdir.name}';
...
function d = filtered_dir(full_filter,re_filter,ex_filter)
...
if exist(full_filter,'dir')
	c = cell(0,1);
	dfiles = struct('name',c,'date',c,'bytes',c,'isdir',c,'datenum',c);
else
...
% exclusion filter
if nargin > 2 && ~isempty(ex_filter)
	if ispc
		no_match = ~cellfun('isempty',regexpi({dfiles.name},ex_filter));
	else
		no_match = ~cellfun('isempty',regexp({dfiles.name},ex_filter));
	end
	dfiles(no_match) = [];
end
% Set navigator style:
